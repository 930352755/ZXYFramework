...
public class Reporter : MonoBehaviour
{
	...
	public void Show(System.Action closeCallBack = null )
    {
		show = true;
	...
	void drawStack()
	{
		...
		if (selectedLog != null) {
			...
			if (GUILayout.Button("复制", GUILayout.Width(size.x * 4), GUILayout.Height(size.y))) {
				GUIUtility.systemCopyBuffer = selectedLog.condition;
			}
			if (GUILayout.Button("关闭", GUILayout.Width(size.x * 4), GUILayout.Height(size.y))) {
				Hide();
			}
			GUILayout.FlexibleSpace();
			GUILayout.EndHorizontal();
	...
	void doShow()
	{
		...
		ReporterGUI rgui = gameObject.GetComponent<ReporterGUI>();
		if (rgui == null)
		{
